...
namespace RadioDld
{
	...
	internal class UpdateCheck
	{
		...
		{
			if (RadioDld.My.Settings.LastCheckForUpdates.AddDays(1) > DateAndTime.Now) {
				return false;
			}
			...
			RadioDld.My.Settings.LastCheckForUpdates = DateAndTime.Now;
			...
			if (versionInfo != string.Empty) {
				...
				if (versionInfo > RadioDld.My.MyProject.Application.Info.Version.ToString()) {
					return true;
				}
			}
