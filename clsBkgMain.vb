...
<System.Runtime.InteropServices.ProgId("clsBkgMain_NET.clsBkgMain")> Public Class clsBkgMain
	Implements RunnableLib.Runnable
	...
	Public Event Error_Renamed(ByVal strError As String, ByVal strCommandOutput As String)
	...
	Public Sub Start(ByVal enmInAction As NextAction, ByVal strInProgType As String, ByVal strInProgID As String, ByVal lngInDuration As Integer, ByVal dteInDate As Date, ByVal strInTitle As String)
		...
		'UPGRADE_WARNING: Couldn't resolve default property of object Me. Click for more: 'ms-help://MS.VSExpressCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
		modAsyncStart.Start(Me)
	End Sub
	
	' This is called from modAsyncStart after control has passed back.
	Private Sub Runnable_Start() Implements RunnableLib.Runnable.Start
	...
	Private Sub ActDownload()
		...
		If booSuccess Then
			...
			RaiseEvent Finished()
		Else
			RaiseEvent Error_Renamed("Download Error", strOutput)
		End If
	End Sub
	...
	Private Sub ActToWav()
		...
		If booSuccess Then
			...
			RaiseEvent Finished()
		Else
			RaiseEvent Error_Renamed("Decoding Error", strOutput)
		End If
	End Sub
	...
	Private Sub ActToMp3()
		...
		If booSuccess Then
			...
			RaiseEvent Finished()
		Else
			RaiseEvent Error_Renamed("Error Converting to MP3", strOutput)
		End If
	End Sub
