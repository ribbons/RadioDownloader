...
Public Class clsAutoUpdate
    Private strVerInfoURL As String
    Private strNewVersionURL As String
    Private strDownloadUrlPrefix As String
    Private thrDownloadThread As New Thread(AddressOf DownloadUpdate)

    Public Sub New(ByVal strVersionInfoURL As String, ByVal strNewVerUrlPrefix As String)
        strVerInfoURL = strVersionInfoURL
        strDownloadUrlPrefix = strNewVerUrlPrefix
    End Sub
    ...
    Public Sub CheckForUpdates()
        If My.Settings.UpdateDownloaded = False Then
            If thrDownloadThread.IsAlive = False Then
                ...
                Try
                    Dim stmReader As New System.IO.StreamReader(webUpdate.OpenRead(strVerInfoURL))
                    ...
                    If strVersionInfo.Length > 0 Then
                        ...
                        If strSplitInfo.GetUpperBound(0) > 0 Then ' Make sure that we have at least two items in the array
                            If strSplitInfo(0) > My.Application.Info.Version.ToString Then ' There is a new version available
                                strNewVersionURL = strDownloadUrlPrefix + strSplitInfo(1)
    ...
    Public Sub InstallUpdate()
        If My.Settings.UpdateDownloaded Then
            If File.Exists(GetAppDataFolder() + "\Radio Downloader Update.msi") Then
                Process.Start("msiexec /i """ + GetAppDataFolder() + "\Radio Downloader Update.msi"" REINSTALL=ALL REINSTALLMODE=vomus", "/S")
            End If
    ...
    Private Sub DownloadUpdate()
        Dim webUpdate As New WebClient
        Try
            webUpdate.DownloadFile(strNewVersionURL, GetAppDataFolder() + "\Radio Downloader Update.msi")
        Catch expWeb As WebException
